...
class ProcessBuilder {
    ...
    _constructJVMArguments112(mods, tempNativePath) {
        ...
        args.push("-cp")
        args.push(
            this.classpathArg(mods, tempNativePath).join(ProcessBuilder.getClasspathSeparator()),
        )

        // Java Arguments
        if (process.platform === "darwin") {
            args.push("-Xdock:name=AdventureLifeLauncher")
                ...
                if (argDiscovery.test(args[i])) {
                    ...
                    switch (identifier) {
                        ...
                        case "natives_directory":
                            ...
                            break
                        case "launcher_name":
                            val = args[i].replace(argDiscovery, "AdventureLife-Launcher")
                            break
