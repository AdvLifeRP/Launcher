...
function updateSelectedServer(serv) {
    ...
    server_selection_button.innerHTML =
        "\u2022 " + (serv != null ? serv.rawServer.name : "No Server Selected")
    if (getCurrentView() === VIEWS.settings) {
...
document.addEventListener("keydown", e => {
    if (newsActive) {
        if (e.key === "ArrowRight" || e.key === "ArrowLeft") {
            document
                .getElementById(e.key === "ArrowRight" ? "newsNavigateRight" : "newsNavigateLeft")
                .click()
        }
...
function displayArticle(articleObject, index) {
    ...
    Array.from(newsArticleContentScrollable.getElementsByClassName("bbCodeSpoilerButton")).forEach(
        v => {
            v.onclick = () => {
